...
int main(void)
{
  ...
  enable_Sim(15);
    ...
    get_And_Send_Current_Time(5);
    // if(strcmp(UART1_Rx_Data, uart1_rx) != 0){
    //   memcpy(uart1_rx,UART1_Rx_Data,100);
    //   HAL_UART_Transmit(&huart3 ,(uint8_t *)&UART1_Rx_Data,strlen(UART1_Rx_Data), 200);
    // }
	}
}
...
void get_And_Send_Current_Time(int time_template){
  ...
  if(HAL_GetTick() - old_time_get_data >= time_template*1000/1.5 && variable_Check_Is_Done == 1)
  {
  ...
  if(HAL_GetTick() - old_time_send_data >= time_template*1000 && variable_Check_Is_Done == 1)
  {
    memcpy(code_Send, AT_Transmit_Data_To_TCP.Send,100);
		couter_Message++;
    if(couter_Message%10 < 1 && couter_Message%100/10 < 1){ don_vi = 0x00; }
    else{ don_vi = couter_Message %10 + 48; }
    if(couter_Message%100/10 < 1 && couter_Message/100 < 1){ chuc = 0x00; }
    else{ chuc = couter_Message %100/10 + 48; }
    if(couter_Message%1000/100 < 1){ tram = 0x00; }
    else{ tram = couter_Message/100 + 48;}
    
    memcpy(data_time_to_send, code_Send,strlen(code_Send));
    memcpy(data_time_to_send + strlen(data_time_to_send), Message,strlen(Message));
    ...
    HAL_UART_Transmit(&huart3, (uint8_t *) &send_time, strlen(send_time),200);
...
{    
  ...
  switch (current_Number_Of_Check_Module_List)
  {
    ...
    case Command_11:
    if (handle_Check_Data_Module(AT_Connect_TCP_Server) ==1) { current_Number_Of_Check_Module_List = Command_12; }
...
int handle_Check_Data_Module(AT_Command_HandleTypeDef AT_Command){
  ...
  memcpy(ready_buffer,AT_Command.Ready,50);
  ...
  if(UART3_Rx_Data[strlen(UART3_Rx_Data)-3] == 83 || UART3_Rx_Data[strlen(UART3_Rx_Data)-3] == 75 ||
  UART3_Rx_Data[strlen(UART3_Rx_Data)-3] == 48 || UART3_Rx_Data[strlen(UART3_Rx_Data)-3] == 49){
